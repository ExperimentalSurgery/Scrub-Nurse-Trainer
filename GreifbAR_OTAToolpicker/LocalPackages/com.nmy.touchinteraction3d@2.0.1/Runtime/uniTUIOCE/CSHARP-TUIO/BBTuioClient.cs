...
namespace TUIO
{
	...
	public class TuioClient
	{
		...
		public bool haveLocalClient;
		private bool foundNonLocalSource=false;
		...
		private void processMessage(OSCMessage message) {
			...
			if (address == "/tuio/2Dobj") {
				...
				if (command == "set") {
					...
					// some hack to determine whether we received any packages from a local TUIO client
					// NOTE this is not foolproof! For example, Multitaction does not send source packages,
					// although it's remote. But as we don't want it to be considered as remote, it's fine... ;-)
					string s_id = (string)args[1];
					if(s_id!=null && s_id.Contains("@"))
						foundNonLocalSource=true;
					...
					if (!lateFrame) {
						...
						// some hack to determine whether we received any packages from a local TUIO client
						if(!foundNonLocalSource)
							haveLocalClient=true;
						foundNonLocalSource=false;
