...
namespace OSC.NET
{
	...
	abstract public class OSCPacket
	{
		...
		{
#if UNITY_FLASH
			byte[] flashArray=new byte[value.Length];
			for(int i=0;i<value.Length;i++)
				flashArray[i]=(byte)value[i];
			return flashArray;
#else
...
#if UNITY_FLASH
			string s="";
			for(int index = start ; bytes[index] != 0 ; index++, count++) s+=bytes[index];
#else
...
#if UNITY_FLASH
        // HACK, until Flash export is fixed
